...
def get_image_size():
    gestures_path = "gestures"  # Base folder

    # Iterate through A-Z to find an image
    for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        letter_folder = os.path.join(gestures_path, letter)
        if os.path.exists(letter_folder):  # Check if the folder exists
            image_files = glob.glob(os.path.join(letter_folder, "*.jpg"))  # Get all JPG images
            if image_files:  # If images are found
                img = cv2.imread(image_files[0], 0)  # Read the first image
                if img is not None:
                    return img.shape  # Return the shape of the first valid image

    raise FileNotFoundError("❌ ERROR: No valid images found in the gestures folder!")
...
def keras_process_image(img):
    # Ensure resizing to match model input shape
    img = cv2.resize(img, (50, 50))  
    img = np.array(img, dtype=np.float32)
    img = np.reshape(img, (1, 50, 50, 1))  # Ensure correct shape for model
    return img
